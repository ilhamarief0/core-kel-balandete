...
var KTJabatanEdit = (function () {
    const modalElement = document.getElementById("kt_modal_edit_jabatan");
    const form = modalElement.querySelector("#kt_modal_edit_jabatan_form");
    ...
    const showSuccessNotification = function (message) {
        Swal.fire({
            text: message,
            icon: "success",
    ...
    if (submitButton) {
        submitButton.addEventListener("click", function (event) {
            ...
            const jabatanId = $('#edit_jabatan_id').val();
            const handleFormSubmit = function () {
                submitButton.setAttribute("data-kt-indicator", "on");
                submitButton.disabled = true;

                const formData = new FormData(form);
                formData.append('_method', 'PUT'); // Penting untuk Laravel PUT request

                $.ajax({
                    type: "POST", // Menggunakan POST untuk PUT request di Laravel
                    url: "jabatan/" + jabatanId, // URL disesuaikan dengan rute
                    data: formData,
                    processData: false,
                    contentType: false,
                    headers: {
        ...
        console.error("Submit button for edit jabatan not found");
    }
