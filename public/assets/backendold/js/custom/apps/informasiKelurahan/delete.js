...
var KTNewsDelete = (function () {
    const DeleteUrl = 'informasiKelurahan/delete';
    ...
    const handleDeleteSingle = function () {
        $(document).on('click', '.delete-user', function (e) {
            ...
            Swal.fire({
                text: `Apakah Anda yakin ingin menghapus news ini?`,
                icon: "warning",
                showCancelButton: true,
                buttonsStyling: false,
                confirmButtonText: "Ya, hapus!",
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        success: function (response) {
                            if (response.status === 'success') {
                                showSuccess(response.message);
                                $(".kt_table_news").DataTable().ajax.reload(null, false);
                            } else {
                                showError(response.message || "An unknown success response occurred.");
                            }
                        },
                        error: function (xhr) {
                            const errorMsg = xhr.responseJSON?.error || "Gagal menghapus news. Silakan coba lagi.";
                            showError(errorMsg);
                        }
                    });
                }
