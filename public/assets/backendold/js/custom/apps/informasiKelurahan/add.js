...
var KTNewsAdd = (function () {
    const form = document.getElementById("kt_modal_add_informasiKelurahan_form");
    const submitButton = form.querySelector('[data-kt-informasiKelurahan-add-action="submit"]');
        ...
        upload() {
            return this.loader.file
                .then(file => new Promise((resolve, reject) => {
                    ...
                    $.ajax({
                        url: '/uploadckEditorImage', // Ini adalah URL ke controller Laravel Anda
                        method: 'POST',
                        data: formData,
                        processData: false,
                        contentType: false,
                        headers: {
                            // PERBAIKI: Mengambil token dari meta tag 'csrf-token'
                        ...
                        xhr: function() {
                            ...
                            xhr.upload.addEventListener('progress', evt => {
                                if (evt.lengthComputable) {
                                    ...
                                    // Anda bisa menambahkan logika progress bar di sini jika diperlukan
                                }
                            }, false);
                            return xhr;
                        },
    ...
    // --- Akhir CKEditor Custom Upload Adapter ---


    const handleFormSubmit = function () {
        if (!submitButton) {
            console.error("Submit button for news add not found.");
            return;
        }
            ...
            success: function (response) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        window.location.href = response.redirect_url || '/backend/informasiKelurahan';
                    }
                });
            },
            error: function (xhr) {
    ...
    return {
        init: function () {
            console.log("KTNewsAdd.init() called.");
            ClassicEditor
